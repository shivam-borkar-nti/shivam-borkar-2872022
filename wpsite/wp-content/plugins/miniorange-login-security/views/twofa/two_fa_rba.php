    <?php
$setup_dirName = $mo2f_dirName .DIRECTORY_SEPARATOR.'views'.DIRECTORY_SEPARATOR.'twofa'.DIRECTORY_SEPARATOR.'link_tracer.php';
...
<div  id = "premium_feature_phone_lost">
    <h3>What happens if  is lost, discharged or not with me
    ...
    <input type="checkbox" class="option_for_auth" name="mo2f_all_users_method" value="1" checked="checked" disabled>OTP over EMAIL
        ...
        foreach($wp_roles->role_names as $id => $name)
        {
            ...
            if(empty($copt[$id])){
             ...
             <span class="mo2f_display_tab mo2f_btn_premium_features" style="padding: 7px 25px;"     ID="mo2f_role_<?php echo $id ?>" onclick="displayTab('<?php echo esc_attr($id) ?>');" value="<?php echo esc_attr($id) ?>" <?php if(get_site_option('mo2f_all_users_method')){echo 'hidden';}?>> <?php echo esc_attr($name) ?></span>
        ...
        foreach($wp_roles->role_names as $id => $name) {    
                ...
                $newcopt=$copt[$id];
                ?>
                <table class="mo2f_for_all_roles" id="mo2f_for_all_<?php echo esc_attr($id) ?>" hidden><tbody>
                <tr>
                <td>
                ...
                <td>
                <input type='checkbox' name="<?php echo esc_attr($id) ?>[]"  value='SMS' <?php echo (in_array("SMS", $newcopt)) ? 'checked="checked"' : '';  ?> disabled /><?php echo __('OTP Over SMS','miniorange-2-factor-authentication');?>&nbsp;&nbsp;
                </td>
                <td>
                <input type='checkbox' name="<?php echo esc_attr($id) ?>[]"  value='PHONE VERIFICATION' <?php echo (in_array("PHONE VERIFICATION", $newcopt)) ? 'checked="checked"' : '';  ?> disabled /><?php echo __('Phone Call Verification','miniorange-2-factor-authentication');?>&nbsp;&nbsp;
                </td>
                </tr>
                <tr>
                <td>
                <input type='checkbox' name="<?php echo esc_attr($id) ?>[]"  value='SOFT TOKEN' <?php echo (in_array("SOFT TOKEN", $newcopt)) ? 'checked="checked"' : '';  ?> disabled /><?php echo __('Soft Token','miniorange-2-factor-authentication');?>&nbsp;&nbsp;
                </td>
                <td>
                <input type='checkbox' name="<?php echo esc_attr($id) ?>[]"  value='MOBILE AUTHENTICATION' <?php echo (in_array("MOBILE AUTHENTICATION", $newcopt)) ? 'checked="checked"' : '';  ?> disabled /><?php echo __('QR Code Authentication','miniorange-2-factor-authentication');?>&nbsp;&nbsp;
                </td>
                <td>
                <input type='checkbox' name="<?php echo esc_attr($id) ?>[]"  value='PUSH NOTIFICATIONS' <?php echo (in_array("PUSH NOTIFICATIONS", $newcopt)) ? 'checked="checked"' : '';  ?> disabled /><?php echo __('Push Notifications','miniorange-2-factor-authentication');?>&nbsp;&nbsp;
                </td>
                </tr>
                <tr>
                <td>
                <input type='checkbox' name="<?php echo esc_attr($id) ?>[]"  value='GOOGLE AUTHENTICATOR' <?php echo (in_array("GOOGLE AUTHENTICATOR", $newcopt)) ? 'checked="checked"' : '';  ?> disabled /><?php echo __('Google Authenticator','miniorange-2-factor-authentication');?>&nbsp;&nbsp;
                </td>
                <td>
                ...
                <td>
                <input type='checkbox' name="<?php echo esc_attr($id) ?>[]"  value='KBA' <?php echo (in_array("KBA", $newcopt)) ? 'checked="checked"' : '';  ?> disabled /><?php echo __('Security Questions (KBA)','miniorange-2-factor-authentication');?>&nbsp;&nbsp;
                </td>
                </tr>
                <tr>
                <td>
                <input type='checkbox' name="<?php echo esc_attr($id) ?>[]"  value='SMS AND EMAIL' <?php echo (in_array("SMS AND EMAIL", $newcopt)) ? 'checked="checked"' : '';  ?> disabled /><?php echo __('OTP Over SMS And Email','miniorange-2-factor-authentication');?>&nbsp;&nbsp;
                </td>
                <td>
                <input type='checkbox' name="<?php echo esc_attr($id) ?>[]"  value='OTP_OVER_EMAIL' <?php echo (in_array("OTP_OVER_EMAIL", $newcopt)) ? 'checked="checked"' : '';  ?> disabled /><?php echo __('OTP Over Email','miniorange-2-factor-authentication');?>&nbsp;&nbsp;
                </td>
                </tr>
                </tbody>
                </div>
...
function mo2f_tooltip_array($mo2f_addon_feature){
    return  '<div class="mo2f_tooltip_addon">
            ...
            <span class="mo2f_tooltiptext_addon" >'. esc_attr($mo2f_addon_feature) .'
            </span>
        </div>';
}
