<?php
	...
	if(current_user_can( 'manage_options' )  && isset($_POST['option']) )
	{
      ...
      if(!get_site_option("admin_email_address_status")|| get_site_option("admin_email_address") ==''){
     	update_site_option('mo_wpns_enable_ip_blocked_email_to_admin','0');
       $notify_admin_on_ip_block = get_site_option('mo_wpns_enable_ip_blocked_email_to_admin') 	 ? "" : "unchacked";
   }
	$notify_admin_on_ip_block 	   = get_site_option('mo_wpns_enable_ip_blocked_email_to_admin') 	 ? "checked" : "";
	$notify_admin_unusual_activity = get_site_option('mo_wpns_enable_unusual_activity_email_to_user') ? "checked" : "";

	$template1					   = get_site_option('custom_admin_template') ? get_site_option('custom_admin_template') : $template1;
	...
	$template2					   = get_site_option('custom_user_template') ? get_site_option('custom_user_template') : $template2;
        ...
        function wpns_handle_admin_email($postValue)
	{
        ...
        if(validate_email($_POST['admin_email_address'])){
			...
			update_site_option('admin_email_address',$postValue['admin_email_address']);
			update_site_option( 'admin_email_address_status', $admin_email_address_status);
	...
	function wpns_handle_notify_admin_on_ip_block($postValue)
	{
		...
		update_site_option( 'mo_wpns_enable_ip_blocked_email_to_admin', $enable_ip_blocked_email_to_admin);
	...
	function wpns_handle_notify_unusual_activity($postValue)
	{
		...
		update_site_option( 'mo_wpns_enable_unusual_activity_email_to_user', $enable_unusual_activity_email_to_user);
	...
	function wpns_handle_custom_template($template1,$template2=null)
	{
		if(!is_null($template1))
			update_site_option('custom_user_template', stripslashes($template1));

		if(!is_null($template2))
			update_site_option('custom_admin_template', stripslashes($template2));
