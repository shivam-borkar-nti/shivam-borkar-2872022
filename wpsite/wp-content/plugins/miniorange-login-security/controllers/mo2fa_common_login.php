...
function mo2f_get_forgotphone_form( $login_status, $login_message, $redirect_to, $session_id_encrypt ) {
	$mo2f_forgotphone_enabled     = get_site_option( 'mo2f_enable_forgotphone' );
					...
					<?php if ( $mo2f_forgotphone_enabled ) {
						if ( isset( $login_message ) && ! empty( $login_message ) ) { ?>
                            <div id="otpMessage" class="mo2fa_display_message_frontend">
                                <p class="mo2fa_display_message_frontend"><?php echo esc_attr($login_message); ?></p>
                            </div>
						<?php } ?>
                        ...
                        <form name="f" id="mo2f_submitkba_loginform" method="post">
                            <div id="mo2f_kba_content">
                                <p style="font-size:15px;">
									...
									echo esc_attr($kba_questions[0]); ?><br>
							...
							<?php if ( $mo2f_login_policy && $mo2f_remember_device_enabled ) {
								?>
...
function mo2f_get_push_notification_oobemail_prompt( $id, $login_status, $login_message, $redirect_to, $session_id_encrypt, $cookievalue ) {
	...
	$mo2f_enable_forgotphone = get_site_option( 'mo2f_enable_forgotphone' );
    ...
    $_SESSION['txid']        = sanitize_text_field($_SESSION['mo2f_transactionId']);
	?>
    <html>
    <head>
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
    ...
    <form name="f" id="mo2f_mobile_validation_form" method="post" class="mo2f_display_none_forms">
        ...
        <input type="hidden" name="redirect_to" value="<?php echo esc_attr($redirect_to); ?>"/>
        <input type="hidden" name="session_id" value="<?php echo esc_attr($session_id_encrypt); ?>"/>
        <input type="hidden" name="TxidEmail" value="<?php echo esc_attr($_SESSION['txid']); ?>"/>
    ...
    <script>
        ...
        if(is_onprem == 1)
        {
            ...
            {   calls = calls + 1;
                var data = {'txid':'<?php echo esc_attr($_SESSION["txid"]);?>'};
                    jQuery.ajax({
                    url: '<?php echo esc_attr (get_site_option("siteurl")); ?>'+"/wp-login.php",
                    type: "POST",
                    data: data,
...
function mo2f_get_qrcode_authentication_prompt( $login_status, $login_message, $redirect_to, $qrCode, $session_id_encrypt , $cookievalue) {
                ...
                <div class="mo2f_modal-body center">
                    ...
                    <div id="scanQRSection">
                        <div style="margin-bottom:10%;">
                            <center>
                                <p class="mo2f_login_prompt_messages"><?php echo mo2f_lt( 'Identify yourself by scanning the QR code with miniOrange Authenticator app.<br> If you are not able to scan the QR code please select the phone is offline option and use the Soft token from the miniornage app.' ); ?></p>
                            </center>
                        </div>
                        <div id="showQrCode" style="margin-bottom:10%;">

                            <center><?php echo '<img src="data:image/jpg;base64,' . sanitize_text_field($qrCode) . '" />'; ?></center>
                        </div>
                        <span style="padding-right:2%;">
                           <center>
    ...
    <form name="f" id="mo2f_backto_mo_loginform" method="post" action="<?php echo wp_login_url(); ?>"
        ...
        <input type="hidden" name="miniorange_mobile_validation_failed_nonce"
               value="<?php echo wp_create_nonce( 'miniorange-2-factor-mobile-validation-failed-nonce' ); ?>"/>
        <input type="hidden" name="session_id" value="<?php echo sanitize_text_field($session_id_encrypt); ?>"/>
    </form>
    <form name="f" id="mo2f_mobile_validation_form" method="post" class="mo2f_display_none_forms">
        <input type="hidden" name="miniorange_mobile_validation_nonce"
               value="<?php echo wp_create_nonce( 'miniorange-2-factor-mobile-validation-nonce' ); ?>"/>
