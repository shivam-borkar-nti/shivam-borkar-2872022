...
class MocURL
{
	...
	function send_otp_token($auth_type, $phone, $email)
	{
		...
		$fields  	 = array('customerKey' 	  => $customerKey,
					'email' 	  => $email,
					'phone' 	  => $phone,
					'authType' 	  => $auth_type,
	...
	global $user;
		...
		$query = '[WordPress Multi Factor Authentication Plugin: ' . sanitize_text_field($customer_feature) . ' - V '.MO2F_VERSION.']: ' . sanitize_text_field($message);
        $content='<div >Hello, <br><br>Ticket ID:'."R".rand(0,1000).'<br><br>First Name :'.sanitize_text_field($user->user_firstname).'<br><br>Last  Name :'.sanitize_text_field($user->user_lastname).'   <br><br>Company :<a href="'.sanitize_text_field($_SERVER['SERVER_NAME']).'" target="_blank" >'.sanitize_text_field($_SERVER['SERVER_NAME']).'</a><br><br>Phone Number :'.sanitize_text_field($phone).'<br><br>Email :<a href="mailto:'.sanitize_text_field($email).'" target="_blank">'.sanitize_text_field($email).'</a><br><br>Query :'.sanitize_text_field($query).'</div>';
